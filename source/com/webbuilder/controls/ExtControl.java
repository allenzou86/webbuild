...
public class ExtControl extends FrontControl {
	...
	protected void createScript() throws Exception {
		...
		if (hasParent) {
			...
			if (setRender && !isTouchControl) {
				if (createInstance || xtype.isEmpty())
					headerScript(getJson("renderTo:Ext.getBody()"));
				else
					headerScript(getJson(xtype + ",renderTo:Ext.getBody()"));
			} else {
				if (createInstance || xtype.isEmpty())
					headerScript(getJson(null));
				else
					headerScript(getJson(xtype));
			}
		}
		if (hasChild) {
	...
	public String getJson(String moreProperties) throws Exception {
		...
		HashSet<String> rkSet = new HashSet<String>();

		rkSet.add("xwlMeta");
		rkSet.add("children");
		rkSet.add("autoCreate");
		if (reserved != null)
