...
public class Query extends BackControl {
	...
	private String id, jndi, sql, orgSql, an, type, transaction, isolation;
	private boolean loadData, uniqueUpdate, debugMode;
	...
	public void create() throws Exception {
		...
		debugMode = Var.getBool("webbuilder.debugMode");
		if (debugMode) {
		...
		request.setAttribute(id + "@st", statement);
		if (paramSize > 0)
			regParameters();
		if (!StringUtil.isEmpty(an))
			...
			if (debugMode)
				printSql();
			if (StringUtil.isEqual(type, "query")) {
		...
		for (i = 0; i < j; i++) {
			...
			for (k = 0; k < l; k++) {
				...
				if (jo.has((String) types[1])) {
					valObj = JsonUtil.opt(jo, (String) types[1]);
					DbUtil.setObject(statement, k + 1, (Integer) types[0],
							valObj);
					if (debugMode)
						paramValList.set(k, StringUtil.stringOf(valObj));
				}
			}
			if (debugMode)
				printSql();
	...
	private void regParameters() throws Exception {
		...
		for (i = 0; i < paramSize; i++) {
			...
			if (callStatement != null
				...
				typeMap[i] = params;
				obj = ga((String) params[1]);
				DbUtil.setObject(statement, i + 1, (Integer) params[0], obj);
				outParamList.add("");
				if (debugMode)
					paramValList.add(StringUtil.stringOf(obj));
			}
		}
	}
	...
	private void replaceSqlParameters() {
		...
		while ((startPos = sql.indexOf("{?")) > -1
			...
			if (debugMode)
				orgSql = StringUtil.replaceFirst(orgSql, "{?" + param + "?}",
						"'{?}'");
			paramList.add(param);
		}
