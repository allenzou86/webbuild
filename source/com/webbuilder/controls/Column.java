...
public class Column extends ExtControl {
	...
	protected String getTagProperties() throws Exception {
		String render = gs("renderer"), align = gs("align"), format = gs("format"), keyName = gs("keyName");
		...
		if (!keyName.isEmpty()) {
			String keyVarName, keyId;
			keyId = "key@@" + keyName;
			keyVarName = (String) request.getAttribute(keyId);
			if (keyVarName == null) {
				keyVarName = "KEY__" + gs("id");
				initScript(StringUtil.concat("Wd.", keyVarName, "=", SysMap
						.getList(keyName, null, false), ";"));
				request.setAttribute(keyId, keyVarName);
			}
			render = StringUtil.concat("return Wb.mapKey(value,", keyVarName,
					");");
		}
		if (render.isEmpty()) {
			if (!hasChild) {
