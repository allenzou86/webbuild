...
public class DataProvider extends FrontControl {
	...
	private void outputJsonArray(ResultSet rs, Integer total,
		...
		buf.append(",\"success\":true,\"metaData\":{\"fields\":");
		...
		while (rs.next()) {
			...
			for (i = 0; i < j; i++) {
				...
				if (hasKey && obj != null && keyMap.has(names[i]))
					obj = SysMap
	...
	private void output(String script) throws Exception {
		String id = gs("id");
		StringBuilder buf = (StringBuilder) request.getAttribute(xwlId
				+ "__script");
		boolean addComma = true;
		boolean directOutput = gb("directOutput", true);

		if (buf == null && directOutput) {
			WebUtil.response(response, script);
			return;
		}
		if (buf == null) {
			buf = new StringBuilder("{");
			addComma = false;
			request.setAttribute(xwlId + "__script", buf);
		}
		if (addComma)
			buf.append(",");
		buf.append("\"").append(id).append("\":").append(script);
		if (directOutput) {
			buf.append("}");
