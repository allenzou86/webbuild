...
public class Store extends ExtControl {
	...
	protected String getTagProperties() throws Exception {
		...
		if (proxy.isEmpty()) {
			...
			if (!isTree)
				addExpress("reader",
						"{type:\"json\",root:\"rows\",rootProperty:\"rows\",totalProperty:\"total\"}");
			except = gs("exception");
	...
	protected String getTagEvents() throws Exception {
		String loadFuncPara, beforeload = gs("beforeload"), msk = ge("mask"), msg = ge("message"), load = gs("load");
		...
		if (beforeload.isEmpty())
			beforeload = "if(!Wb.setStore(store))return false;";
		else
			beforeload = StringUtil.concat(
