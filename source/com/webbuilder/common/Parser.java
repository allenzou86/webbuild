...
public class Parser {
	...
	public void parse() throws Exception {
		...
		tokens = gs("tokens");
		touchMode = gb("touchMode", false);
		if (!tokens.isEmpty() && checkToken(tokens))
			loginRequired = false;
		try {
			storeVar();
			if (loginRequired) {
				if (WebUtil.checkLogin(request, response, touchMode)) {
	...
	private void createChildren(JSONObject obj) throws Exception {
		...
		for (i = 0; i < j; i++) {
			...
			control.isTouchControl = touchMode;
	...
	private void createBody() throws Exception {
		...
		if (loadLib) {
			...
			if (touchMode) {
				cssFiles
						.add("webbuilder/controls/touch/resources/css/sencha-touch.css");
				cssFiles.add("webbuilder/css/style.css");
				jsFiles.add(StringUtil.concat(
						"webbuilder/script/locale/wb-lang-", Str
								.optLanguage(lang), ".js"));
				jsFiles.add(StringUtil.concat(
						"webbuilder/controls/touch/locale/t-lang-", Str
								.optTouchLanguage(lang), ".js"));
				jsFiles.add("webbuilder/controls/touch/sencha-touch-all.js");
				jsFiles.add("webbuilder/script/wb.js");
				appendScript(initScript, StringUtil.concat("Wb.initTouch(",
						ajaxTimeout, ",", Integer.toString(zoneOffset), ");"));
			} else {
				cssFiles
						.add("webbuilder/controls/ext/resources/css/ext-all.css");
				cssFiles.add("webbuilder/css/style.css");
				jsFiles.add(StringUtil.concat(
						"webbuilder/script/locale/wb-lang-", Str
								.optLanguage(lang), ".js"));
	...
	private boolean createModule(boolean create) throws Exception {
		...
		isCreateBody = gb("createBody", true);
		if (isCreateBody)
	...
	private String getTouchViewport() throws Exception {
		JSONArray ja = content.optJSONArray("children");
		if (ja == null)
			return "";
		JSONObject jo = JsonUtil.findObject(ja, "xwlMeta", "tviewport");
		if (jo == null)
			return "";
		else {
			ExtControl control = new ExtControl();
			JSONObject meta = XwlBuffer.getMeta("tviewport");
			...
			control.response = response;
			control.xwlObject = jo;
			control.hasChild = false;
			control.parentControl = content;
			control.hasParent = false;
			control.isFirstChild = true;
			return StringUtil.concat("\nviewport:{\n", control.getJson(null),
					"\n},");
		}
	}
	...
	private void outputScript(boolean suffix) throws Exception {
		...
		if (l1 + l2 + l3 > 0) {
			...
			if (loadLib)
				if (touchMode)
					header.append(StringUtil.concat("\nExt.setup({",
							getTouchViewport(), "\nonReady:function(){"));
				else
