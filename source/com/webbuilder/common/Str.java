...
public class Str {
	...
	public static String optTouchLanguage(String lang) throws Exception {
	...
	public static synchronized void initialize(boolean reload) throws Exception {
		...
		fs = new File(Main.path, "webbuilder/controls/touch/locale")
				.listFiles();
		for (File f : fs) {
			name = f.getName();
			if (StringUtil.isSame(StringUtil.substring(name, 0, 6), "t-lang")) {
